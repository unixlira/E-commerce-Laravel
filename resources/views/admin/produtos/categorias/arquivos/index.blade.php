...
@section('content')
	<div class="row">
        ...
        <div style="float: right;">
            <a href="{{url('admin/produtos/categorias/arquivos/create/0')}}" class="btn btn-default btn.app" style="margin-left: 75%;"><i class="fa fa-plus"></i> Adicionar </a>
        </div>
 	</div>
 	<div class="row">
 		<div class="x_panel">
            ...
            <table class="table table-striped">
				...
				@foreach($arquivos as $arquivo)
 					<tr>
                         <td>{{$arquivo->nome}}</td>
						 <td>{{$arquivo->tipo}}</td>
						 <td>{{$arquivo->categoria->nome or ""}}</td>
                        <td style="text-align: right;">
 							<a href="{{url('admin/produtos/categorias/arquivos/create/'.$arquivo->id)}}" class="btn btn-default"><i class="fa fa-edit"></i> Editar</a>
							 <a href="{{url('admin/produtos/categorias/arquivos/delete/'.$arquivo->id)}}" class="btn btn-default" onclick="return confirm('Você tem certeza que deseja Excluir?')"><i class="fa fa-trash"></i> Excluir</a>
							 <a href="{{ assetsCliente($arquivo->arquivo) }}" target="_blank" class="btn btn-default"><i class="fa fa-trash"></i> Donwload</a>							 
 						</td>
 					</tr>
 				@endforeach
                </tbody>
            </table>
        <div class="col-md-offset-5">{!! $arquivos->render() !!}</div>
 		</div>
 	</div>
@endsection